...
class FeedManager:
    ...
    async def load_opml(self) -> Tuple[int, Dict[str, str]]:
        ...
        try:
            ...
            validation_results = await self.feed_validator.validate_feeds(feeds_to_validate)
            ...
            if self.deleted_file.exists():
                ...
                for feed_data in result.feeds:
                    feed = Feed(
                        ...
                        genre=getattr(feed_data, 'category', 'Other'),
                        description=getattr(feed_data, 'description', ''),
                        deleted_at=datetime.fromisoformat(getattr(feed_data, 'deletedAt', datetime.now().isoformat()))
                    )
