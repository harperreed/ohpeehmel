...
class TestDisplay(unittest.TestCase):
    ...
    def test_display_feeds(self, mock_console):
        feeds = {
            "hash1": Feed(
                title="Feed 1", url="http://example.com/feed1", genre="Technology"
            ),
            "hash2": Feed(
                title="Feed 2", url="http://example.com/feed2", genre="Science"
            ),
    ...
    def test_display_latest_articles(self, mock_parse, mock_console):
        mock_parse.return_value = MagicMock(
            entries=[
                {
                    "title": "Article 1",
                    "published_parsed": datetime(2023, 1, 1).timetuple(),
                },
                {
                    "title": "Article 2",
                    "published_parsed": datetime(2023, 1, 2).timetuple(),
                },
                {
                    "title": "Article 3",
                    "published_parsed": datetime(2023, 1, 3).timetuple(),
                },
            ]
        )
    ...
    def test_display_feeds_special_characters(self, mock_console):
        feeds = {
            ...
            "hash3": Feed(
                title="Feed > 3", url="http://example.com/feed3", genre="News"
            ),
        }
        display_feeds(feeds)
    ...
    def test_display_feeds_long_text_truncation(self, mock_console):
        ...
        feeds = {
            "hash1": Feed(
                title=long_title, url="http://example.com/feed1", genre="Technology"
            )
        }
        display_feeds(feeds)
